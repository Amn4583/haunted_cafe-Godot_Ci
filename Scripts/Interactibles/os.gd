...
@onready var scanner_app = $Apps/scanner
@onready var mouse_click = $"../mouse_click"
...
func _on_scanner_button_pressed() -> void:
	_open_app(scanner_app)


func _open_app(app: Control):
	mouse_click.play(0.4)
...
func _on_scanner_close_pressed() -> void:
	close_app(scanner_app)
