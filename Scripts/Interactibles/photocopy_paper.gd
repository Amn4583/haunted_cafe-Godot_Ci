extends Node3D
@export var can_pickable: bool = true
var paper_code: String = ""
var copies: int = 0
...
func cleanup_after_drop():
	is_held = false
	EventsManager.emit_scan_request(paper_code, copies)
...
func set_code(code_text, copies_needed):
	paper_code = code_text
	copies = copies_needed
	print("Set code:", code_text)
	print("Set copies:", copies_needed)
...
func can_pickup():
	if can_pickable:
		...
		can_pickable = true
		set_meta("interaction_name", "Document Form")
